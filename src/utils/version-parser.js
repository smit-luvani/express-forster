/**
 * @see {@link https://github.com/asamuzaK/semverParser}
 */
 ...
 * @returns {boolean} - result
 */
const isValidSemanticVersion = (version, strict = false) => {
    if (!isString(version)) {
 ...
 */
const compareSemanticVersion = (version, base, strict = false) => {
    if (!isString(version)) {
        throw new TypeError(`Expected String but got ${getType(version)}.`);
    }
    if (!isString(base)) {
    ...
    if (!isValidSemanticVersion(version, !!strict)) {
        throw new Error(`${version} is not valid version string.`);
    }
    if (!isValidSemanticVersion(base, !!strict)) {
 ...
 */
const parseSemanticVersion = (version, strict = false) => {
    if (!isString(version)) {
        throw new TypeError(`Expected String but got ${getType(version)}.`);
    }
    const matches = isValidSemanticVersion(version, !!strict);
...
module.exports = { compareSemanticVersion, isValidSemanticVersion, parseSemanticVersion };